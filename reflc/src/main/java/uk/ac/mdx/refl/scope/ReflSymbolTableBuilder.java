...
public class ReflSymbolTableBuilder extends eFrmBaseListener {

    public static Symbol.Type getType(final int tokenType) {
        switch (tokenType) {
            case eFrmParser.SUBFORM:
                return Symbol.Type.tSUBFORM;
            case eFrmParser.ID:
    ...
    public static Symbol.Type getArrayType(final int tokenType) {
        switch (tokenType) {
            ...
            case eFrmParser.ID:
                return Symbol.Type.tSUBFORM_REF_ARRAY;
        }
        return Symbol.Type.tINVALID;
    }
    ...
    @Override
    public void enterSubformDecl(final SubformDeclContext ctx) {
        final String name = ctx.ID().getText();
        final int typeTokenType = ctx.start.getType();
        final Symbol.Type type = getType(typeTokenType);

        // push new scope by making new one that points to enclosing scope
        final SubformSymbol group = new SubformSymbol(name, type, currentScope);
    ...
    @Override
    public void exitSubformDecl(final SubformDeclContext ctx) {
        System.out.println(currentScope);
    ...
    void defineField(final FieldDeclContext varTypeCtx, final Token nameToken) {
        ...
        if (varTypeCtx.ARRAY() != null) {
            ...
            final int size = Integer.parseInt(varTypeCtx.INT().getText());
            if (type == Type.tSUBFORM_REF_ARRAY) {
