...
import main.antlr.eFrmParser.ErrorStatContext;
import main.antlr.eFrmParser.ExprContext;
...
import uk.ac.mdx.refl.scope.VariableSymbol;

public class ReflGeneratingVisitor extends eFrmBaseVisitor<String> {

    private final ParseTreeProperty<Scope> scopes;
    private Scope currentScope;

    public ReflGeneratingVisitor(final ParseTreeProperty<Scope> scopes) {
        super();
        this.scopes = scopes;
    }
    ...
    private final Map<String, SubformDeclContext> groups = new HashMap<String, SubformDeclContext>();

    private SubformDeclContext getSubformContext(final String id) {
        return groups.get(id);
    }

    private String genCle(final FieldDeclContext fdc) {
        final ST st = group.getInstanceOf(fdc.ARRAY() == null ? "cle" : "arrayCle");
        ...
        if (s instanceof ArrayFieldSymbol) {
            ...
            for (int i = 0; i < size; i++) {
                if (s.getCustomTypeName() != null) {
                    // Inline rendering of subform
                    final SubformDeclContext gdc = groups.get(s.getCustomTypeName());
    ...
    @Override
    public String visitSubformType(final SubformTypeContext ctx) {
        return "new " +
            getSubformContext(ctx.ID().getText()).ID().getText() +
    ...
    @Override
    public String visitAndExpr(final AndExprContext ctx) {
        return visit(ctx.expr(0)) + " && " + visit(ctx.expr(1));
    }

    @Override
    public String visitOrExpr(final OrExprContext ctx) {
        return visit(ctx.expr(0)) + " || " + visit(ctx.expr(1));
    }
    ...
    @Override
    public String visitErrorStat(final ErrorStatContext ctx) {
        final StringBuilder sb = new StringBuilder();
        for (final ExprContext ec : ctx.expr()) {
            sb.append(visit(ec) + ".addError('err" + ++errNum + "','" + sanitiseString(ctx.STRING().getText())
                + "');\n");
        }
        return sb.toString();
    }
